...
RUN rm -v /initializer/initialize-builtin.sh /initializer/default && echo '* * * * * /usr/bin/php7.3 /config/www/protected/yii queue/run >/dev/null 2>&1' > /var/spool/cron/crontabs/root && echo '* * * * * /usr/bin/php7.3 /config/www/protected/yii cron/run >/dev/null 2>&1' >> /var/spool/cron/crontabs/root && crontab /var/spool/cron/crontabs/root
